...
-- The RLS relies on 'sys_membership_cache'. If it's empty, you see nothing.
...
BEGIN
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'sys_membership_cache') THEN
        ...
        -- Insert fresh data from vve_memberships
        INSERT INTO sys_membership_cache (user_id, vve_id, role)
        SELECT user_id, vve_id, role
...
DROP POLICY IF EXISTS "Board and Admins can manage all profiles" ON profiles;
